...
class MADDPG(Model):
    ...
    def get_actions(
        self, state, status, exploration, actions_avail, target=False, last_hid=None
    ):
        target_policy = self.target_net.policy if self.args.target else self.policy
        if self.args.continuous:
            means, log_stds, hiddens = (
                self.policy(state, last_hid=last_hid)
                if not target
                else target_policy(state, last_hid=last_hid)
            )
            if means.size(-1) > 1:
    ...
    def get_loss(self, batch):
        ...
        next_values = (
            self.target_net.value(next_state, next_actions.detach())
            .contiguous()
            .view(-1, self.n_)
        )
