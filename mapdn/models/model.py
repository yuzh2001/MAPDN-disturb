...
class Model(nn.Module):
    ...
    def update_target(self):
        for name, param in self.target_net.policy_dicts.state_dict().items():
            update_params = (
                1 - self.args.target_lr
            ) * param + self.args.target_lr * self.policy_dicts.state_dict()[name]
    ...
    def transition_update(self, trainer, trans, stat):
        if self.args.replay:
            ...
            replay_cond = (
                trainer.steps > self.args.replay_warmup
    ...
    def episode_update(self, trainer, episode, stat):
        if self.args.replay:
            ...
            replay_cond = (
                trainer.episodes > self.args.replay_warmup
                and len(trainer.replay_buffer.buffer) >= self.args.batch_size
    ...
    def policy(
        self, obs, schedule=None, last_act=None, last_hid=None, info={}, stat={}
    ):
        # obs_shape = (b, n, o)
        ...
        if self.args.agent_id:
            agent_ids = (
                th.eye(self.n_).unsqueeze(0).repeat(batch_size, 1, 1).to(self.device)
            )  # shape = (b, n, n)
            obs = th.cat((obs, agent_ids), dim=-1)  # shape = (b, n, n+o)

        if self.args.shared_params:
            # print (f"This is the shape of last_hids: {last_hid.size()}")
            obs = obs.contiguous().view(
                batch_size * self.n_, -1
            )  # shape = (b*n, n+o/o)
    ...
    def train_process(self, stat, trainer):
        ...
        for t in range(self.args.max_steps):
            ...
            action, action_pol, log_prob_a, _, hid = self.get_actions(
                state_,
                status="train",
                exploration=True,
                actions_avail=th.tensor(trainer.env.get_avail_actions()),
                target=False,
                last_hid=last_hid,
            )
            ...
            _, next_action_pol, _, _, _ = self.get_actions(
                next_state_,
                status="train",
                exploration=True,
                actions_avail=th.tensor(trainer.env.get_avail_actions()),
    ...
    def evaluation(self, stat, trainer):
        ...
        for _ in range(num_eval_episodes):
            ...
            for t in range(self.args.max_steps):
                ...
                action, _, _, _, hid = self.get_actions(
                    state_,
                    status="test",
                    exploration=False,
                    actions_avail=th.tensor(trainer.env.get_avail_actions()),
    ...
    def unpack_data(self, batch):
        ...
        last_step = (
            th.tensor(batch.last_step, dtype=th.float)
            .to(self.device)
            .contiguous()
            .view(-1, 1)
        )
        done = (
            th.tensor(batch.done, dtype=th.float)
            .to(self.device)
            .contiguous()
            .view(-1, 1)
        )
        action = th.tensor(np.concatenate(batch.action, axis=0), dtype=th.float).to(
            self.device
        )
        log_prob_a = th.tensor(np.concatenate(batch.action, axis=0), dtype=th.float).to(
            self.device
        )
        value = th.tensor(np.concatenate(batch.value, axis=0), dtype=th.float).to(
            self.device
        )
        next_value = th.tensor(
            np.concatenate(batch.next_value, axis=0), dtype=th.float
        ).to(self.device)
        state = prep_obs(list(zip(batch.state))).to(self.device)
        next_state = prep_obs(list(zip(batch.next_state))).to(self.device)
        action_avail = th.tensor(np.concatenate(batch.action_avail, axis=0)).to(
            self.device
        )
        last_hid = th.tensor(np.concatenate(batch.last_hid, axis=0), dtype=th.float).to(
            self.device
        )
        hid = th.tensor(np.concatenate(batch.hid, axis=0), dtype=th.float).to(
            self.device
        )
